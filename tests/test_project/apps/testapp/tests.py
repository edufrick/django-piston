...
from test_project.apps.testapp.models import ListFieldsModel
...
class BasicAuthTest(MainTests):
    def test_invalid_auth_header(self):
        ...
        response = self.client.get("/api/entries/", HTTP_AUTHORIZATION=bad_auth_string)
        self.assertEquals(response.status_code, 401)
    ...
    def test_both_authenticators(self):
        for username, password in (
            ...
            self.assertEquals(
                ...
                200,
                "Failed with combo of %s:%s" % (username, password),
            )
...
class MultiXMLTests(MainTests):
    ...
    def test_singlexml(self):
        ...
        result = self.client.get(
            "/api/entry-%d.xml" % (obj.pk,), HTTP_AUTHORIZATION=self.auth_string
        ).content
        self.assertEquals(expected, result)
...
class AbstractBaseClassTests(MainTests):
    ...
    "some_field": "something here"
        ...
        for id_ in ids:
            result = self.client.get(
                "/api/abstract/%d.json" % id_, HTTP_AUTHORIZATION=self.auth_string
            ).content
...
class ValidationTest(MainTests):
    def test_basic_validation_fails(self):
        ...
        self.assertEquals(
            resp.content,
            'Bad Request <ul class="errorlist">'
...
class PlainOldObject(MainTests):
    def test_plain_object_serialization(self):
        ...
        self.assertEquals(resp.status_code, 200)
        self.assertEquals(
            {"type": "plain", "field": "a field"}, simplejson.loads(resp.content)
        )
...
class Issue188ValidateWithFiles(MainTests):
    ...
    def test_upload_with_file(self):
        ...
        tmp_fs.seek(0)
        resp = self.client.post(
            reverse("file-upload-test"), data={"chaff": "pewpewpew", "le_file": tmp_fs}
        )
        self.assertEquals(resp.status_code, 200)
        self.assertEquals(
...
class EmitterFormat(MainTests):
    def test_format_in_url(self):
        resp = self.client.get("/api/entries.json", HTTP_AUTHORIZATION=self.auth_string)
        self.assertEquals(resp.status_code, 200)
        self.assertEquals(resp["Content-type"], "application/json; charset=utf-8")
        resp = self.client.get("/api/entries.xml", HTTP_AUTHORIZATION=self.auth_string)
        self.assertEquals(resp.status_code, 200)
        self.assertEquals(resp["Content-type"], "text/xml; charset=utf-8")
        resp = self.client.get("/api/entries.yaml", HTTP_AUTHORIZATION=self.auth_string)
        self.assertEquals(resp.status_code, 200)
        self.assertEquals(resp["Content-type"], "application/x-yaml; charset=utf-8")
    ...
    def test_format_in_accept_headers(self):
        ...
        self.assertEquals(resp["Content-type"], "application/json; charset=utf-8")
        resp = self.client.get(
            "/api/entries/", HTTP_AUTHORIZATION=self.auth_string, HTTP_ACCEPT="text/xml"
        )
        self.assertEquals(resp.status_code, 200)
        self.assertEquals(resp["Content-type"], "text/xml; charset=utf-8")
        resp = self.client.get(
